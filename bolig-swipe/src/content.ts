...
const container = document.getElementById('Bolig') as HTMLDivElement | null;
let startX = 0;
...
function setSwipeEvents(card: HTMLDivElement) {
    ...
    const handleMove = (e: TouchEvent | MouseEvent) => {
        ...
        if (offsetX > 0) container.classList.add('right-swipe');
        else if (offsetX < 0) container.classList.add('left-swipe');
    };
